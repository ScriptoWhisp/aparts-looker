...
| 7. Database Migration | 6/7 | In Progress|  |
...
- [x] 07-01-PLAN.md — SQLAlchemy engine + Listing model + Alembic baseline + Postgres container + entrypoint.sh (DB-01, DB-02, DB-03, DB-06)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 07-02-PLAN.md — Rewire data_store.py against SQLAlchemy; preserve public API; `_lock` becomes no-op shim; port existing tests to db_session fixture (DB-05, DB-07)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 07-03-PLAN.md — migrate_from_json.py one-shot idempotent loader + entrypoint invocation (DB-04)
...
- [x] 07-05-PLAN.md — Hot-caller optimizations: settings recompute, backfill-costs, backfill-commutes, delete_all_listings, cost_override, cost_override_reset (DB-05)
